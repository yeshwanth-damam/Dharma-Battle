## Product
**Dharma Battle** — a 2D top-down arena shooter mobile game inspired by Indian mythology, built with Expo React Native + FastAPI + MongoDB. Solo wave shooter plus optional **co-op multiplayer** (2–4 players) via an authoritative FastAPI WebSocket room server. Not a PUBG-scale online battle royale.

## Target Users
Casual mobile gamers, Indian mythology enthusiasts, arcade shooter fans. Play Store + App Store distribution.

## Core Gameplay Loop
1. Enter warrior name → 2. Pick hero + astra + map in lobby → 3. Battle waves of AI enemies (solo) **or** invite allies to a co-op room → 4. Earn coins + XP → 5. Unlock new heroes/astras → 6. Climb leaderboard.
...
- **Lobby**: player card (level, XP bar, coins), loadout slots (hero / astra / map), solo play + **co-op multiplayer** entry.
...
- **Co-op Multiplayer**: create/join room by code, ready-up, host start; server-authoritative 20Hz sim; shared waves; ally HP strip; rewards on match end.
...
## Multiplayer API (`/api/mp/*`)
- `POST /mp/rooms` — create co-op room (returns room code)
- `POST /mp/rooms/join` — join by code
- `GET /mp/rooms` — list open lobby rooms
- `GET /mp/rooms/{code}` — room summary
- `WS /mp/ws/{code}?player_id=` — lobby sync + authoritative snapshots (`ready`, `input`, `snapshot`, `match_end`)
...
- Solo battle loop runs at 60fps via `requestAnimationFrame` with mutable refs.
- Co-op: clients are thin renderers; `backend/multiplayer.py` owns positions, combat, waves. Fixed logical arena 800×500 scaled to device.
- Future scale path: Colyseus/Nakama or dedicated room pods; `react-native-skia` if entity counts grow. Not a Unity/Unreal rewrite.
