...
async def apply_match_rewards(player_id: str, kills: int, survived_seconds: int, victory: bool) -> Optional[dict]:
    """Grant coins/xp/level/stats for a completed match. Shared by the REST
    ``/match/complete`` endpoint and the real-time co-op server. Returns the
    updated player document, or ``None`` if the player no longer exists."""
    doc = await db.players.find_one({"id": player_id}, {"_id": 0})
    if not doc:
        return None
...
async def complete_match(res: MatchResult):
    await _fetch(res.player_id)  # ensure player exists (404 otherwise)
    doc = await apply_match_rewards(res.player_id, res.kills, res.survived_seconds, res.victory)
    if not doc:
        raise HTTPException(status_code=404, detail="Player not found")
    return Player(**doc)
...
# The authoritative WebSocket room server lives in ``realtime.py`` and is
# mounted under the same ``/api`` prefix (endpoint: ``/api/ws/battle``).
realtime.configure(persist_match=apply_match_rewards)
realtime.register_routes(api_router)
