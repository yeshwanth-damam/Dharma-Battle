...
from game_data import HEROES, WEAPONS, MAPS, STRIPE_CATALOG, COIN_PACKS
from rewards import apply_match_result, fetch_player as _rewards_fetch_player
from multiplayer import create_multiplayer_router
...
async def _fetch(player_id: str) -> dict:
    return await _rewards_fetch_player(db, player_id)
...
async def complete_match(res: MatchResult):
    doc = await apply_match_result(db, res.player_id, res.kills, res.survived_seconds, res.victory)
    return Player(**doc)
...
app.include_router(create_multiplayer_router(db), prefix="/api")
